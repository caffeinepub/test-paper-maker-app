import { Paper } from '../../state/mockData';
...
export function convertPaperToText(paper: Paper): string {
  ...
  paper.sections.forEach((section, sectionIdx) => {
    lines.push(`SECTION ${sectionIdx + 1}: ${section.title || `${section.marks} Marks`}`);
    lines.push('');
    
    // Headings
    section.headings.forEach((heading, headingIdx) => {
      if (heading.title) {
        lines.push(`  ${heading.title} (${heading.plannedCount || 0} questions)`);
        lines.push('');
      }
      ...
      headingQuestions.forEach((question, qIdx) => {
        ...
        } else if (question.questionType === 'fill-in-blank' && question.fillInBlankData) {
          lines.push(`      [Fill in the blank]`);
        } else if (question.questionType === 'true-false') {
          lines.push(`      [True/False]`);
        }
        
        lines.push(`      [${question.marks} mark${question.marks !== 1 ? 's' : ''}]`);
        lines.push('');
      });
    });
