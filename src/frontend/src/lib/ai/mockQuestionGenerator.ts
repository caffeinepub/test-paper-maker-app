...
function extractCleanTopic(focusArea: string): string {
  ...
  const cleanedTopic = focusArea
    .toLowerCase()
    .replace(/\b(numerical|conceptual|theory|theoretical|practical|type|questions?|for|standard|std|class|grade|level|10th|9th|8th|7th|6th|11th|12th)\b/gi, '')
    .replace(/\s+/g, ' ')
    .trim();
...
function detectNumericalIntent(focusArea: string): boolean {
  const numericalKeywords = /\b(numerical|calculate|compute|find|solve|value|number|quantity|measurement|problem)\b/i;
  return numericalKeywords.test(focusArea);
}

function getRandomTemplate(difficulty: string, marks: number, questionStyle: 'numerical' | 'conceptual'): string {
  const templates = questionStyle === 'numerical' 
    ? numericalTemplates[difficulty as keyof typeof numericalTemplates]?.[marks as keyof typeof numericalTemplates.easy]
    : conceptualTemplates[difficulty as keyof typeof conceptualTemplates]?.[marks as keyof typeof conceptualTemplates.easy];
...
export function generateMockQuestions(params: GenerateQuestionsParams): GeneratedQuestion[] {
  ...
  let questionStyle: 'numerical' | 'conceptual' = params.questionStyle || 'conceptual';
